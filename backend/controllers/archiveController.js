...
const searchArchive = asyncHandler(async function (req, res) {
    ...
    if (sort === 'random') {
        ...
        items = sampled.map(toRecentPicture);
        total = overallTotal;
        totalPages = 1;
        effectivePage = 1;
    } else {
        ...
        items = docs.map(toRecentPicture);
        total = count;
...
const getFacets = asyncHandler(async function (req, res) {
    const filter = { display: true };

    const [tags, cameraModels, cities, states, countries, galleries] = await Promise.all([
        Media.distinct('tags', filter),
        Media.distinct('metadata.cameraModel', filter),
        Media.distinct('location.city', filter),
        Media.distinct('location.state', filter),
        Media.distinct('location.country', filter),
        Gallery.find({ display: true }).select('slug title').sort({ sortOrder: 1 }).lean()
    ]);
    ...
    res.json({
        ...
        galleries: galleries.map(function (g) { return { slug: g.slug, title: g.title }; })
    });
});

module.exports = { searchArchive, getFacets };
